...
/**
 * Die Klasse SchenkenController bildet die Controllerklasse zur FXML-Datei "schenke-view.fxml"
 * und beinhaltet die noetigen Methoden und Variablen zur Behandlung von Nutzereingaben
 * und Anpassung der Gui an diese.
 *
 * @author Felix Ahrens.
 */
public class SchenkenController extends StadtController
{
    ...
    /**
     * Methode zum Anzeigen der Pane "charakterDisplay". Ueber die Methode
     * "gebeCharakterAusID" wird auf den zugehoerigen Charakter zugegriffen.
     * Das Aktualisieren der Charakterwerte im "charakterDisplay" wird ueber die
    ...
    /**
     * Methode, die das Anheuern steuert. Aufgerufen wird diese ueber einen Klick
     * auf den "anheuernButton"-Knopf. Das Anheuern geschieht nur, wenn genug Gold im Besitz ist.
     *
     * @pre Die Methode "fuehreTransaktionDurchWennMoeglich" muss existieren und von der
     * Klasse "SchenkenController" aus zugaenglich sein. Die GUI-Elemente muessen
     * in der FXML-Datei "schenke-view.fxml" vorhanden sein und in dieser Controllerklasse ebenso.
     * Auf die Interfaces fuer Konstanten und Strings muss zugegriffen werden koennen.
     *
     * @post Der boolesche Wert "istAngeheuert" des ausgewaehlten Charakters wurde
     * auf true gesetzt, wenn sich die spielende Person den Charakter leisten konnte.
     * Ueber die GUI ist eine visuelle Bestaetigung getaetigt worden.
     *
     * @author Felix Ahrens.
     */
    @FXML
    ...
    /**
     * Methode zum Aktualisieren des CharakterDisplays. Es werden die Methoden
     * "zeigeCharakterAnheuernButtonSinnvoll" und "setzeLabelTexte" aufgerufen,
     * beiden wird die Instanz der Klasse Charakter, die der Methode schon beim Aufruf
     * uebergeben wurde, uebergeben. Beide Methoden steuern GUI-Elemente im "charakterDisplay".
     * Zusaetzlich wird der Charakter, der der Methode uebergeben wurde,
     * als Klassenvariable des SchenkenControllers gesetzt.
     *
    ...
    /**
     * Methode, die, abhaengig davon, ob der die booleschen Werte "istImBesitz" und
     * "transaktionIstMoeglich" des uebergebenen Charakters true oder false ist,
     * den Knopf mit der ID "anheuernButton" in der FXML-Datei "schenke-view.fxml" aktiviert
     * oder deaktiviert. Grund dafuer ist, dass man einen gekauften Charakter nicht
     * nochmal kaufen koennen soll und der Versuch des Kaufens unterbunden werden soll,
     * wenn man sich den Charakter nicht leisten kann.
     *
     * @param charakter Der Charakter, der in der Schenke gerade ausgewaehlt ist und
     * der in der "charakterDisplay"-Pane angezeigt wird.
     *
     ...
     *
     * @pre Die Klasse Charakter muss die Variablen besitzen. Die Methode muss einen
     * gueltigen Charakter uebergeben bekommen. Die Labels muessen mit den korrekten
     * IDs in der Klasse SchenkenController und in der FXML-Datei "schenke-view.fxml" existieren.
     *
     * @post Die Labels in der "schenke-view.fxml" zeigen die Werte an, die den Werten
     * des Charakters entsprechen, der der Methode uebergeben wurde.
     *
     * @author Felix Ahrens.
     */
    ...
    /**
     * Methode, die einer uebergebenen Pane einen Charakter zuordnet.
     * Dabei wird die ID der Pane ueber eine switch-Anweisung mit Konstanten verglichen
     ...
     *
     * @pre Die Singleton-Instanz muss gesetzt sein, die ID der Pane muss mit
     * einem der vier Konstanten-Strings aus der switch-Anweisungen uebereinstimmen.
     *
     * @post Es wurde der zur ID der uebergebenen Pane gehoerender Charakter aus
     * dem Spielstand (GameFile) zurueckgegeben.
     *
     * @author Felix Ahrens.
     */
