...
/**
 * Die Klasse MissionMemoryController ist die Controllerklasse fuer die FXML Datei "mission-memory-view.fxml".
 *  In ihr befinden sich alle Methoden, die die Mission spielbar machen. Dazu gehoeren Spiellogische Methoden und solche,
 *  die die GUI veraendern.
 * @Author Felix Ahrens
 */
public class MissionMemoryController extends ControllerController
{
    ...
    /**
     * Methode zum Behandeln des "umdrehens" eines Steins.
     *  Dabei wird ueberprueft, ob das der erste von zwei jeweils "umdrehbaren" Steinen ist. Ist das der Fall, wird der Stein
     *  nur in einer Klassenvariable gespeichert und dessen ID der spielenden Person ausgegeben.
     *  Ist schon ein Stein als "umgedreht" gespeichert, wird dieser mit dem nun "umgedrehten" Stein verglichen.
     *  Haben beide den gleichen Kennbuchstaben an festgelegter Stelle, wird das Paar ueber den Methodenaufruf "updateSpielInfoLabel"
     *  im entsprechenden Label fuer die anwendende Person sichtbar gespeichert und beide Steine (AnchorPanes) auf invisible gesetzt.
     * @pre Die GUI-Elemente, Methoden und Klassen muessen existieren und verwendbar sein.
     * @post Es wurde entweder die eine ID des ersten Steins angezeigt oder beide Steine invisible gesetzt, dafuer die IDs im "SpielInfoLabel" angezeigt und beide invisible gemacht oder beide Steine auf "nicht umgedreht" zurueckgesetzt.
     ...
     * @post Das "SpielInfoLabel" zeigt die bisher gesammelten Steinpaare an. Sind das alle (also Acht), wird ueber den Methodenaufruf beendeMissionWennFertig
     *  die Mission beendet.
    ...
    /**
     * Methode, die die Mission beendet. Diese wird von dem Beenden-Button aufgerufen, wenn die Mission abgeschlossen wurde.
