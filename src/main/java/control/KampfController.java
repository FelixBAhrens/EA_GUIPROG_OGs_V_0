...
/**
 * Kampfcontroller, der saemtliche Methoden zum Kaempfen beinhaltet. Dazu gehoeren GUI-Manipulationsmethoden, Methoden
 * zum Verwalten und ausfuehren von Kampfhandlungen, Initialisierungsmethoden und weitere. Der Kampfcontroller ist die
...
{
    /**
     * Enum fuer den Kampftyp. Wird verwendet, um der Controllerklasse den Kampftyp mitzuteilen, bevor die
     * kampf-FXML-Datei aufgerufen wird.
     *
     * @Author Felix Ahrens
     */
     ...
     * @post Der "kampfStartDialog" wurde invisible gesetzt und, abhaengig vom Kampftyp, eine Methode zum
     * Starten/Initialisieren eines bestimmten Kampfes aufgerufen
     * @Author David Kien, Felix Ahrens
     */
    @FXML
    ...
    /**
     * Methode, um den Endgegnerkampf zu initialisieren. Diese erstellt und setzt die Kaempfer und ruft Methoden auf, um
     * Charaktere und Gegner zu initialisieren, diese dem kaempferArray hinzuzufuegen, die Timeline zu berechnen, die
     * Charakterposition, die angezeigte Timeline und die "KampfAnchorPanes" zu aktualisieren. Ebenso wird die Methode
    ...
    /**
     * Methode, die am Ende eines Zuges aufgerufen wird, und abhaengig von dem naechsten Element der Queue den naechsten
     * Zug aufruft. Ist die Queue leer, wird der Kampf mit einem Unentschieden beendet, wenn kein Kaempfer gestorben
     * ist.
     */
    ...
    /**
     * Methode, um den Stand-Alone-Kampf ueber Netzwerk zu starten. Bei einem Arenakampf ist die Szenerie anders. Da die
     * Spiellogik und das Netzwerk zu diesem Zeitpunkt aber noch nicht durchdacht ist, kann die GUI hierfuer nicht
     * naeher entwickelt werden.
     *
     ...
     *
     * @param sieger Die Instanz der Klasse Kaempfef, die im Kampf als Sieger festgestellt wurde, und der Methode als
     *               "Sieger" uebergeben wurde.
     * @pre Die Strings muessen Interface enthalten sein, der KampfEndeDialog muss als AnchorPane in der Klasse
     * vorhanden sein.
     ...
     * @pre Die Strings muessen Interface enthalten sein, der KampfEndeDialog muss als AnchorPane in der Klasse
     * vorhanden sein.
     * @post Der "kampfEndeDialog" wurde mit "Unentschieden" ausgegeben. Die darauf folgende Szene wurde als Stadt
     * "angemeldet".
     * @Author Felix Ahrens
     */
     ...
     * <p>
     * Diese Methode wird aufgerufen, um den initialen Zustand der Benutzeroberfläche zu setzen. Sie macht das
     * Dialogfenster "kampfStartDialog" sichtbar und fügt einen Listener hinzu, der auf Änderungen der Szene des
     * "gridPane" reagiert. Wenn eine neue Szene gesetzt wird, wird ein Event-Handler registriert, der Tastendrücke in
     * der Szene behandelt.
     *
     * @param location  Die URL, die zum Auflösen relativer Pfade für die Hauptcontainer der Benutzeroberfläche
     *                  verwendet wird
     * @param resources Die Ressourcen, die zur Lokalisierung der Hauptcontainer der Benutzeroberfläche verwendet
     *                  werden
    ...
    /**
     * Methode, die den von der nutzenden Person spielbaren Zug steuert. In dieser Methode werden Nutzereingaben
     * freigeschaltet, die nur gemacht werden können, wenn der Mensch am Zug ist. Das Gegenteil dazu ist die Methode
     * "gegnerZug()", die den Gegner (Bot) steuert.
     *
     * @pre "zugBeenden" muss ein Button sein, der sich in der FXML-Datei "kampf-view.fxml" befindet. Der Button muss
     * auch im Contoller sein.
     * @post Der Knopf wurde freigeschaltet und diesem ein Eventlistener hinzugefuegt, der den Zug beendet und die
     * Methode "naechsterZug" aufruft.
     * @Author Felix Ahrens
     */
    @FXML
    ...
    /**
     * Methode, die Methoden zum Bestimmen der Ausfuehrbarkeit von Kampfhandlungen aufruft, um, falls ja, die
     * zugehoerigen Buttons zu aktivieren.
     *
     * @pre Die Methoden und Buttons muessen Existieren und erreichbar sein. Die jeweils aktivierten Buttons duerfen nur
     * die Handlung ausfuehren, die von der jeweiligen Methode, die hier aufgerufen wurde, als machbar bestaetigt
     * wurde.
     * @post Die Buttons, deren zugehoerige Handlung von den Methoden als ausfuehrbar eingestuft wurde, wurden
     * aktiviert.
     * @Author Felix Ahrens
     */
     ...
     * Die Methode "wechseleSzene" in der Klasse SzenenManager muss vorhanden sein und die ihr uebergebene FXML-Datei
     ...
     * @post Der gegnerische Zug wurde ausgefuehrt. Dabei wurde die Timeline aktualisiert, eine Sekunde gewartet, der
     * Gegner bewegt und Attakiert.
     * @Author Felix Ahrens
     */
    ...
    /**
     * Methode, die die Timeline berechnen soll. Tut sie aber nicht, weil ich fuer die GUI verantwortlich bin. Aber ich
     * brauche die Methode, um daraus eben eine Timeline anzeigen zu koennen. Deshalb diese voreingestellte
     * Reihenfolge.
     *
    ...
    /**
     * Methode, die eine Rectangle erstellt und sie der "gridPane" an der fuer den Gegner spezifizierten Position
     * hinzufuegt.
     *
     * @pre
     * @author Felix Ahrens
     ...
     * Parametern aufruft.
     *
     * @pre Die Methode "attackiere" muss existieren und die Kaempfer "spieler" und "gegner" muessen in der Klasse
     * gespeichert sein. Der Kaempfer muss die vom Spieler steuerbare Instanz der Klasse Kaempfer sein.
    ...
    /**
     * Methode zum Wirken von Magie. Dabei wird geprueft, ob das Wirken von Magie ueberhaupt moeglich ist. Falls ja,
     * werden die Manapunkte des Angreifers auf 0 gesetzt und der Schaden berechnet. Der berechnete Schaden wird danach
     * auf den gegner angewendet. Zuletzt werden Methoden zum ueberpruefen des Lebens der Beteiligten und zum
     ...
     * muss diese erst das passende Artefakt herausfiltern. Das geschieht ueber eine Switch-Anweisung. Damit wird die
     * Methode wendeArtefaktAn aufgerufen, dieser wird das jeweilige ausgewaehlte Artefakt uebergeben sowie die
     * Kaempfer-Instanzen "spieler" und "gegner".
     *
     * @param event Das Event, dem der Methodenaufruf entspricht
     * @pre Die Methode muss von einer Pane aufgerufen werden. Deren ID muss aus einem Bezeichner, einem Unterstricht
     * und danach dem Namen des Artefakt {Schwert, Statue, Ring} bestehen. Die verwendeten Methoden, Klassen, Instanzen
    ...
    /**
     * Boolesche Methode, die bestimmt, ob das Wirken von Magie moeglich ist. Das wird anhand der Zahl der Manapunkte
     * festgemacht.
     *
    ...
    /**
     * Methode, die bestimmt, ob Attackieren moeglich ist. Wichtig dafuer ist die Entfernung und die Anzahl der
     * verbleibenden Fernkaempfe.
     *
     ...
     * @return True, wenn die Entfernung zwischen angreifer und verteidiger geringer als drei ist oder geringer als
     * sechs ist und die Zahl verbleibender Fernkaempfe des Angreifers groesser als null(Zahlenwert) ist.
     * @pre Die Methoden muessen existieren und erreichbar sein. "berechneEntfernung" muss eine Double-Zahl
     * zurueckliefern.
    ...
    /**
     * Methode zum Attackieren. Ist das Attackieren moeglich, wird die Methode "verwalteSchaden" aufgerufen, die den
     * Schaden berechnet und austeilt.
     *
    ...
    /**
     * Methode zum Verwalten vom Schaden. Diese berechnet den Schaden, den der Angreifer dem Verteidiger hinzufuegt auf
     * Basis der Attribute fuer Position, Nahkampfwert, Fernkampfwert und Zahl der Fernkaempfe und wendet diesen
     ...
     * @pre Die verwendeten Parameter, Methoden, Konstanten und Instanzvariablen (bzw. Getter und Setter davon) muessen
     * vorhanden und erreichbar sein.
     * @post Der berechnete Schaden wurde auf den Verteidiger angewendet und es wurde ueberprueft, ob dieser nach der
     * Attacke noch lebt.
     * @Author Felix Ahrens
     */
    ...
    /**
     * Methode zum Berechnen der Entfernung. Dabei wird die Entfernung mit dem Satz des Pythagoras berechnet und als
     * Double zurueckgegeben.
     *
     ...
     * @post Die berechnete Entfernung zwischen den in den Instanzen der Klasse Kaempfer gespeicherten wird
     * zurueckgegeben.
     * @Author Felix Ahrens
     */
     ...
     * Fall, wird der Kampf ueber den Methodenaufruf von "beendeKampf" beendet. Der Methode wird der festgestellte
     * Sieger uebergeben.
     *
     * @pre Die genutzten Methoden und Variablen muessen erreichbar sein. Die Strings und Konstanten muessen im
     * jeweiligen Interface vorhanden sein.
     * @post Der Kampf wurde, falls einer der Kaempfer keine Gesundheit mehr hat, beendet. Der Dateiname der
     * entsprechenden Nachkampfszene wurde in die Variable "nachKampfSzenenName" gesetzt.
     * @Author Felix Ahrens
     */
    ...
    /**
     * Methode, mit der der Gegner den kuerzesten Weg zum Spieler nimmt. Da die Spiellogik, insbesondere zu
     * Kampfhandlungen gemacht wurde, wurde diese Methode entwurfen, um wenigstens ganz banale Kampfhandlungen zu
     * ermoegichen.
     *
     ...
     *
     * @pre Die GUI-Elemente muessen in der FXML-Datei und der Klasse existieren. Alle anderen Methoden, Variablen und
     * Instanzen muessen auch existieren und erreichbar sein.
    ...
    /**
     * Methode, die das ArtefaktDisplay aktualisiert und die sich im Besitz befindenden Artefakte in die HBox
     * "artefaktDisplay" als Panes hinzufuegt.
     *
